 ...
 * @since 5.0
 */
public class BJettyRequestUpgradeStrategy implements RequestUpgradeStrategy, Lifecycle {
  ...
  protected static final Log logger = LogFactory.getLog(BJettyRequestUpgradeStrategy.class);
    ...
    logger.trace("start()");
      ...
      logger.trace("start() servletContext: " + servletContext);
			if (!isRunning() && servletContext != null) {
        logger.trace("start() this.running: " + this.running);
				this.running = true;
				try {
    ...
    logger.trace("call startLazily this.servletContext: " + this.servletContext);
		if (this.servletContext != null) {
			return;
		}
    logger.trace("call startLazily synchronized: this.lifecycleMonitor: " + this.lifecycleMonitor);
    logger.trace("call startLazily synchronized: request: " + request);
    logger.trace("call startLazily synchronized: request.getClass(): " + request.getClass());
		synchronized (this.lifecycleMonitor) {
			if (this.servletContext == null) {
				this.servletContext = request.getServletContext();
        logger.trace("call startLazily synchronized: request.getServletContext(): " + this.servletContext);
				start();
			}
		}
	}
