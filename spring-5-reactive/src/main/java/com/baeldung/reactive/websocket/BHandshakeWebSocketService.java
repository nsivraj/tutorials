 ...
 * @since 5.0
 */
public class BHandshakeWebSocketService implements WebSocketService, Lifecycle {
	...
	private static final boolean tomcatPresent = ClassUtils.isPresent(
			...
			BHandshakeWebSocketService.class.getClassLoader());
	...
	protected static final Log logger = LogFactory.getLog(BHandshakeWebSocketService.class);
	 ...
	 */
	public BHandshakeWebSocketService() {
	 ...
	 */
	public BHandshakeWebSocketService(RequestUpgradeStrategy upgradeStrategy) {
	...
	private static RequestUpgradeStrategy initUpgradeStrategy() {
		...
		try {
			...
			Class<?> clazz = ClassUtils.forName(className, BHandshakeWebSocketService.class.getClassLoader());
    ...
    logger.trace("handleRequest exchange: " + exchange.getClass());
    logger.trace("handleRequest handler: " + handler.getClass());

    // StringWriter callingStack = new StringWriter();
    // PrintWriter pw = new PrintWriter(callingStack);
    // new Exception().printStackTrace(pw);
    // logger.trace("who is calling: " + callingStack);

    //org.springframework.http.server.reactive.ServletServerHttpRequest

    ServerHttpRequest request = exchange.getRequest();
    logger.trace("handleRequest request: " + request.getClass());
    //logger.trace("handleRequest request.getServletContext(): " + request.getServletContext());
